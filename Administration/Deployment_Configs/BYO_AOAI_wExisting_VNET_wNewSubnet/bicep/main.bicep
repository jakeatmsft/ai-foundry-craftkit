/*
  ...
  - Reuses an existing virtual network and creates a new private endpoint subnet for private connectivity.
*/
...
@description('Name for the subnet that will be created for the private endpoint inside the existing VNet.')
param newPeSubnetName string = 'foundry-pe-subnet'
...
// Break down the VNet resource ID so we can create child resources within its scope.
...
// Reference the existing virtual network so we can add a subnet for the private endpoint.
...
// Subnet dedicated to the new private endpoint inside the existing virtual network.
resource privateEndpointSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-05-01' = {
  parent: existingVnet
  name: newPeSubnetName
  properties: {
